...
)   -- correction #5 : ) manquant pour fermer raw AS (...)
SELECT
...
"""
    if cfg.mode in (RunMode.OFFLINE, RunMode.PROBE):
        ...
        return count
    s3_delete_prefix(cfg, cfg.bucket_silver, "slv_clients/sites_mission/")
    ddb.execute(
...
"""
    if cfg.mode in (RunMode.OFFLINE, RunMode.PROBE):
        ...
        return count
    s3_delete_prefix(cfg, cfg.bucket_silver, "slv_clients/contacts/")
    ddb.execute(
        f"COPY ({query}) TO '{silver_path}' "
        f"(FORMAT PARQUET, COMPRESSION ZSTD, OVERWRITE_OR_IGNORE TRUE)"
    )
...
"""
    if cfg.mode in (RunMode.OFFLINE, RunMode.PROBE):
        ...
        return count
    s3_delete_prefix(cfg, cfg.bucket_silver, "slv_clients/encours_credit/")
    ddb.execute(
        f"COPY ({query}) TO '{silver_path}' "
        f"(FORMAT PARQUET, COMPRESSION ZSTD, OVERWRITE_OR_IGNORE TRUE)"
    )
