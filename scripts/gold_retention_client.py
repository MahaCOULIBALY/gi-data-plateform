"""gold_retention_client.py — Silver → Gold fact_retention_client + fact_rentabilite_client.
Phase 3 · GI Data Lakehouse · Manifeste v2.0

=== CORRECTIONS SESSION 2 (audit Silver→Gold) ===
- Aliases lowercase Silver (date_facture, type_facture, efac_num, taux_paye/fact, base_paye/fact)

=== CORRECTIONS SESSION 3 (audit Gold) ===
- G-RC-B01 : TRUNCATE avant pg_bulk_insert (idempotence PG)
- G-RC-M01 : Fan-out build_retention_query — missions pré-agrégées par (tie_id,rgpcnt_id,trimestre)
             avant JOIN factures (plus d'inflation montant_ht_calc)
- G-RC-M02 : Fan-out build_rentabilite_query — ca_annee et mission_costs séparés,
             jointure finale par (tie_id, annee)
- G-RC-M03 : RunMode importé + guards OFFLINE/PROBE
- G-RC-M04 : try/except dans run()
- G-RC-m01 : imports sys/logging supprimés
- G-RC-m02 : filter_tables importé et appliqué

# MIGRÉ : iceberg_scan → read_parquet(s3://gi-poc-silver/slv_*) (D01)
"""
import json

from shared import (
    Config, RunMode, Stats,
    get_duckdb_connection, get_pg_connection, pg_bulk_insert,
    filter_tables, logger,
)
from gold_helpers import cte_montants_factures, cte_heures_par_contrat


PIPELINE = "gold_retention_client"


def build_retention_query(cfg: Config) -> str:
    """Suivi fidélisation par trimestre — ML-ready features.
    G-RC-M01 : missions pré-agrégées par (tie_id, rgpcnt_id, trimestre) → pas de fan-out CA.
    """
    return f"""
    WITH factures AS (
        SELECT * FROM read_parquet('s3://{cfg.bucket_silver}/slv_facturation/factures/**/*.parquet')
    ),
    {cte_montants_factures(cfg)},
    dim_clients AS (
        SELECT * FROM read_parquet('s3://{cfg.bucket_silver}/slv_clients/dim_clients/**/*.parquet')
        WHERE is_current = true
        QUALIFY ROW_NUMBER() OVER (PARTITION BY tie_id ORDER BY valid_from DESC NULLS LAST) = 1
    ),
    -- G-RC-M01 : pré-agrégation missions par (tie_id, rgpcnt_id, trimestre) — évite fan-out CA
    missions_agg AS (
        SELECT
            tie_id::INT                                                               AS tie_id,
            rgpcnt_id::INT                                                            AS rgpcnt_id,
            DATE_TRUNC('quarter', TRY_CAST(date_debut AS DATE))                      AS trimestre_mission,
            COUNT(DISTINCT per_id::VARCHAR || '|' || cnt_id::VARCHAR)                AS nb_missions
        FROM read_parquet('s3://{cfg.bucket_silver}/slv_missions/missions/**/*.parquet')
        WHERE date_debut IS NOT NULL AND tie_id IS NOT NULL
        GROUP BY 1, 2, 3
    ),
    quarterly AS (
        SELECT
            c.client_sk,
            f.tie_id::INT                                                             AS tie_id,
            DATE_TRUNC('quarter', TRY_CAST(f.date_facture AS DATE))                  AS trimestre,
            SUM(CASE WHEN f.type_facture='F'
                     THEN COALESCE(mt.montant_ht_calc, 0)::DECIMAL(18,2) ELSE 0 END)
            - SUM(CASE WHEN f.type_facture='A'
                     THEN COALESCE(mt.montant_ht_calc, 0)::DECIMAL(18,2) ELSE 0 END) AS ca_net,
            COALESCE(SUM(ma.nb_missions), 0)                                          AS nb_missions,
            COUNT(DISTINCT f.efac_num)                                                AS nb_factures,
            MAX(TRY_CAST(f.date_facture AS DATE))                                     AS derniere_facture
        FROM factures f
        LEFT JOIN montants     mt ON mt.fac_num  = f.efac_num
        LEFT JOIN dim_clients   c ON c.tie_id    = f.tie_id::INT
        LEFT JOIN missions_agg ma
            ON ma.tie_id          = f.tie_id::INT
           AND ma.rgpcnt_id       = f.rgpcnt_id::INT
           AND ma.trimestre_mission = DATE_TRUNC('quarter', TRY_CAST(f.date_facture AS DATE))
        WHERE f.date_facture IS NOT NULL
        GROUP BY 1, 2, 3
    ),
    with_lag AS (
        SELECT *,
            LAG(ca_net)    OVER (PARTITION BY client_sk ORDER BY trimestre)      AS ca_q1,
            LAG(ca_net, 4) OVER (PARTITION BY client_sk ORDER BY trimestre)      AS ca_yoy,
            LAG(nb_missions) OVER (PARTITION BY client_sk ORDER BY trimestre)    AS missions_q1,
            COUNT(*) OVER (PARTITION BY client_sk ORDER BY trimestre
                           ROWS BETWEEN 3 PRECEDING AND CURRENT ROW)             AS freq_4q
        FROM quarterly
    )
    SELECT
        client_sk, tie_id, trimestre, ca_net,
        COALESCE(ca_net - ca_q1, 0)                                               AS delta_ca_qoq,
        CASE WHEN COALESCE(ca_q1, 0) > 0
             THEN ROUND((ca_net - ca_q1) / ca_q1, 4) ELSE NULL END               AS delta_ca_qoq_pct,
        COALESCE(ca_net - ca_yoy, 0)                                              AS delta_ca_yoy,
        CASE WHEN COALESCE(ca_yoy, 0) > 0
             THEN ROUND((ca_net - ca_yoy) / ca_yoy, 4) ELSE NULL END             AS delta_ca_yoy_pct,
        nb_missions, nb_factures,
        freq_4q                                                                   AS frequence_4_trimestres,
        derniere_facture,
        CURRENT_DATE - derniere_facture                                           AS jours_inactivite,
        CASE
            WHEN CURRENT_DATE - derniere_facture > 180 THEN 'PERDU'
            WHEN CURRENT_DATE - derniere_facture > 90  THEN 'HIGH'
            WHEN CURRENT_DATE - derniere_facture > 45  THEN 'MEDIUM'
            WHEN delta_ca_qoq_pct < -0.3               THEN 'MEDIUM'
            WHEN freq_4q <= 1                           THEN 'MEDIUM'
            ELSE 'LOW'
        END                                                                       AS risque_churn,
        ROUND(LEAST(1.0, GREATEST(0.0,
            0.30 * LEAST((CURRENT_DATE - derniere_facture)::DECIMAL / 180, 1.0)
          + 0.25 * (1.0 - LEAST(freq_4q::DECIMAL / 4, 1.0))
          + 0.25 * CASE WHEN COALESCE(delta_ca_qoq_pct, 0) < 0
                        THEN LEAST(ABS(delta_ca_qoq_pct), 1.0) ELSE 0 END
          + 0.20 * CASE WHEN COALESCE(delta_ca_yoy_pct, 0) < 0
                        THEN LEAST(ABS(delta_ca_yoy_pct), 1.0) ELSE 0 END
        )), 4)                                                                    AS churn_score_ml
    FROM with_lag
    WHERE trimestre IS NOT NULL
    ORDER BY trimestre DESC, ca_net DESC
    """


def build_rentabilite_query(cfg: Config) -> str:
    """Rentabilité nette par client × année.
    G-RC-M02 : ca_annee (depuis factures) et mission_costs (depuis missions) calculés
               séparément pour éviter le fan-out sur (tie_id, rgpcnt_id).
    """
    return f"""
    WITH factures AS (
        SELECT * FROM read_parquet('s3://{cfg.bucket_silver}/slv_facturation/factures/**/*.parquet')
    ),
    {cte_montants_factures(cfg)},
    dim_clients AS (
        SELECT * FROM read_parquet('s3://{cfg.bucket_silver}/slv_clients/dim_clients/**/*.parquet')
        WHERE is_current = true
        QUALIFY ROW_NUMBER() OVER (PARTITION BY tie_id ORDER BY valid_from DESC NULLS LAST) = 1
    ),
    -- G-RC-M02 : CA depuis factures seules, sans JOIN missions — pas de fan-out
    ca_annee AS (
        SELECT
            dc.client_sk,
            f.tie_id::INT                                                             AS tie_id,
            EXTRACT(YEAR FROM TRY_CAST(f.date_facture AS DATE))::INT                  AS annee,
            SUM(CASE WHEN f.type_facture='F'
                     THEN COALESCE(mt.montant_ht_calc, 0)::DECIMAL(18,2) ELSE 0 END)
            - SUM(CASE WHEN f.type_facture='A'
                     THEN COALESCE(mt.montant_ht_calc, 0)::DECIMAL(18,2) ELSE 0 END) AS ca_net,
            SUM(CASE WHEN f.type_facture='F'
                     THEN COALESCE(mt.montant_ht_calc, 0)::DECIMAL(18,2) ELSE 0 END) AS ca_brut
        FROM factures f
        LEFT JOIN montants   mt ON mt.fac_num = f.efac_num
        LEFT JOIN dim_clients dc ON dc.tie_id = f.tie_id::INT
        WHERE f.date_facture IS NOT NULL
        GROUP BY 1, 2, 3
    ),
    -- G-RC-M02 : coûts missions séparément, agrégés par (tie_id, annee)
    {cte_heures_par_contrat(cfg)},
    mission_costs AS (
        SELECT
            m.tie_id::INT                                                             AS tie_id,
            EXTRACT(YEAR FROM TRY_CAST(m.date_debut AS DATE))::INT                    AS annee,
            COALESCE(SUM(hc.h_fact * c.taux_fact::DECIMAL(10,4)), 0)                 AS ca_missions,
            COALESCE(SUM(hc.h_paye * c.taux_paye::DECIMAL(10,4)), 0)                 AS cout_paye,
            COUNT(DISTINCT m.per_id)                                                  AS nb_interimaires
        FROM read_parquet('s3://{cfg.bucket_silver}/slv_missions/missions/**/*.parquet') m
        LEFT JOIN read_parquet('s3://{cfg.bucket_silver}/slv_missions/contrats/**/*.parquet') c
            ON c.per_id = m.per_id AND c.cnt_id = m.cnt_id
        LEFT JOIN heures_par_contrat hc ON hc.per_id = m.per_id AND hc.cnt_id = m.cnt_id
        WHERE m.tie_id IS NOT NULL AND m.date_debut IS NOT NULL
        GROUP BY 1, 2
    )
    SELECT
        ca.client_sk, ca.tie_id, ca.annee, ca.ca_net,
        COALESCE(mc.ca_missions, 0)                                                   AS ca_missions,
        COALESCE(mc.cout_paye, 0)                                                     AS cout_paye,
        COALESCE(mc.ca_missions, 0) - COALESCE(mc.cout_paye, 0)                      AS marge_brute,
        CASE WHEN COALESCE(mc.ca_missions, 0) > 0
             THEN ROUND((COALESCE(mc.ca_missions, 0) - COALESCE(mc.cout_paye, 0))
                        / mc.ca_missions, 4)
             ELSE 0 END                                                               AS taux_marge,
        ca.ca_brut * 0.05                                                             AS cout_gestion_estime,
        COALESCE(mc.ca_missions, 0) - COALESCE(mc.cout_paye, 0) - (ca.ca_brut * 0.05) AS rentabilite_nette,
        CASE WHEN COALESCE(mc.ca_missions, 0) > 0
             THEN ROUND((COALESCE(mc.ca_missions, 0) - COALESCE(mc.cout_paye, 0)
                         - (ca.ca_brut * 0.05)) / mc.ca_missions, 4)
             ELSE 0 END                                                               AS taux_rentabilite_nette,
        COALESCE(mc.nb_interimaires, 0)                                               AS nb_interimaires
    FROM ca_annee ca
    LEFT JOIN mission_costs mc ON mc.tie_id = ca.tie_id AND mc.annee = ca.annee
    WHERE ca.annee IS NOT NULL
    ORDER BY ca.annee DESC, ca.ca_net DESC
    """


def run(cfg: Config) -> dict:
    stats = Stats()
    active = filter_tables(
        ["fact_retention_client", "fact_rentabilite_client"], cfg)  # G-RC-m02

    if cfg.mode in (RunMode.OFFLINE, RunMode.PROBE):  # G-RC-M03
        for name in active:
            logger.info(json.dumps(
                {"mode": cfg.mode.name, "table": name, "action": "skipped"}))
            stats.tables_processed += 1
        return stats.finish(cfg, PIPELINE)

    ret_cols = ["client_sk", "tie_id", "trimestre", "ca_net", "delta_ca_qoq",
                "delta_ca_qoq_pct", "delta_ca_yoy", "delta_ca_yoy_pct",
                "nb_missions", "nb_factures", "frequence_4_trimestres",
                "derniere_facture", "jours_inactivite", "risque_churn", "churn_score_ml"]
    rent_cols = ["client_sk", "tie_id", "annee", "ca_net", "ca_missions", "cout_paye",
                 "marge_brute", "taux_marge", "cout_gestion_estime",
                 "rentabilite_nette", "taux_rentabilite_nette", "nb_interimaires"]
    table_map = {
        "fact_retention_client": (build_retention_query, ret_cols),
        "fact_rentabilite_client": (build_rentabilite_query, rent_cols),
    }

    ret_rows, rent_rows = [], []
    try:  # G-RC-M04
        with get_duckdb_connection(cfg) as ddb:
            ret_rows = ddb.execute(build_retention_query(cfg)).fetchall()
            rent_rows = ddb.execute(build_rentabilite_query(cfg)).fetchall()
            logger.info(json.dumps({
                "fact_retention_client": len(ret_rows),
                "fact_rentabilite_client": len(rent_rows),
            }))
    except Exception as e:
        logger.exception(json.dumps({"step": "duckdb_build", "error": str(e)}))
        stats.errors.append({"step": "duckdb_build", "error": str(e)})
        return stats.finish(cfg, PIPELINE)

    rows_map = {
        "fact_retention_client": ret_rows,
        "fact_rentabilite_client": rent_rows,
    }
    try:  # G-RC-M04
        with get_pg_connection(cfg) as pg:
            for name, (_, cols) in table_map.items():
                if name not in active:
                    continue
                # G-RC-B01 : idempotence PG
                with pg.cursor() as cur:
                    cur.execute(f"TRUNCATE TABLE gld_clients.{name}")
                pg.commit()
                pg_bulk_insert(cfg, pg, "gld_clients", name,
                               cols, rows_map[name], stats)
                stats.tables_processed += 1
                stats.rows_transformed += len(rows_map[name])
    except Exception as e:
        logger.exception(json.dumps({"step": "pg_insert", "error": str(e)}))
        stats.errors.append({"step": "pg_insert", "error": str(e)})

    stats.extra["retention_rows"] = len(ret_rows)
    stats.extra["rentabilite_rows"] = len(rent_rows)
    return stats.finish(cfg, PIPELINE)


if __name__ == "__main__":
    run(Config())
