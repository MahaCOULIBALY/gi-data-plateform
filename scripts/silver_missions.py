...
#   MISS_ANNULE validé probe 2026-03-15 : NULL/0=non-annulé ; 1=12 314 annulés
#   Logique CASE validée : MTFCNT_FINCNT=0→EN_COURS, LIKE '%fin de mission%'→TERME_NORMAL,
#   MTFCNT_FINCNT=1 autres→RUPTURE, fallback LIKE FINMISS_LIBELLE si MTFCNT_ID NULL
# CORRECTIONS (2026-03-23) :
# Anomalie #3 : s3_delete_prefix avant COPY — purge Silver avant réécriture
# Anomalie #5 : ajout ')' fermeture CTE dans _build_wtmiss_sql (wtcmd AS)
#               et _build_fin_mission_sql (pymtfcnt AS)
# Anomalie #8 : extra_nulls — information_schema.columns → DESCRIBE (fiable sur VIEWs DuckDB)
# Bronze v2 stable : MISS_ANNULE présent dans _COLS["WTMISS"] → extra_nulls retiré de FIN_MISSION
"""
import json
...
class _Table:
    ...
    # Colonnes absentes du Bronze à injecter comme NULL dans src.
    # Utiliser avec parcimonie — préférer corriger le Bronze upstream.
...
def _build_fin_mission_sql(cfg: Config) -> str:
    ...
    MISS_ANNULE confirmé Bronze v2 (2026-03-23) : présent dans _COLS["WTMISS"].
    Valeurs probe 2026-03-15 : NULL=2 341 013 / 0=559 375 → non-annulé ; 1=12 314 → annulé.
    """
    b = cfg.bucket_bronze
    dp = cfg.date_partition
    return f"""
...
FROM src
LEFT JOIN wtcnti cnti ON cnti.PER_ID      = src.PER_ID AND cnti.CNT_ID = src.CNT_ID
LEFT JOIN wtfinmiss f ON f.finmiss_code   = TRIM(src.FINMISS_CODE)
LEFT JOIN pymtfcnt mtf ON mtf.mtfcnt_id  = f.mtfcnt_id
WHERE src.PER_ID IS NOT NULL AND src.CNT_ID IS NOT NULL
QUALIFY ROW_NUMBER() OVER (
    PARTITION BY src.PER_ID, src.CNT_ID ORDER BY src._loaded_at DESC
) = 1
...
""",
    ...
    # FIN_MISSION (2026-03-15) — rupture CTT : WTMISS + WTCNTI + WTFINMISS + PYMTFCNT
    # Cible : slv_missions/fin_mission → Gold fact_rupture_contrat
    # Bronze v2 stable (2026-03-23) : MISS_ANNULE présent dans _COLS["WTMISS"] → extra_nulls supprimé
    _Table(
        name="FIN_MISSION",
        bronze="wtmiss",
...
def _process(ddb, cfg: Config, t: _Table, stats: Stats) -> None:
    ...
    try:
        ddb.execute(
            ...
            f"SELECT * FROM read_json_auto('{bronze_path}')")

        if t.extra_nulls:
            # DESCRIBE est fiable sur les VIEWs DuckDB avec table functions (contrairement
            # à information_schema.columns qui retourne souvent vide pour ce type de vue).
            existing = {r[0] for r in ddb.execute("DESCRIBE src").fetchall()}
            missing = {col: typ for col, typ in t.extra_nulls.items()
                       if col not in existing}
            if missing:
                null_cols = ", ".join(
                    f"NULL::{typ} AS {col}" for col, typ in missing.items())
                ddb.execute(
                    f"CREATE OR REPLACE VIEW src AS "
                    f"SELECT *, {null_cols} "
        ...
        # Purge avant écriture — évite l'accumulation de fichiers Parquet (Anomalie #3)
        # s3_delete_prefix gère les guards OFFLINE/PROBE : no-op hors mode LIVE
        ...
        row = ddb.execute(
            f"SELECT COUNT(*) FROM read_parquet('{silver_path}')").fetchone()
        count = row[0] if row else 0
        stats.tables_processed += 1
