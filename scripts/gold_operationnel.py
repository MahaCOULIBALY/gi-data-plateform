...
# ENRICHISSEMENT (2026-03-26) — Phase 1 KPI completion :
# fact_delai_placement : refonte grain (agence_id, semaine_debut) — retrait categorie_delai,
#   ajout pct_moins_4h (<=4h) / pct_meme_jour (<=24h) / pct_plus_3j (>72h)
# fact_conformite_dpae : ajout nb_dpae_conformes (ecart<=0) + nb_dpae_retard (ecart>0),
#   taux_conformite_dpae corrigé = conformes/total (était transmises/total — erroné)
#   ecart_moyen_heures restreint aux missions en retard uniquement
...
def build_delai_placement_query(cfg: Config) -> str:
    """Délai entre commande et début mission — grain (agence_id, semaine_debut).
    Colonnes Silver : delai_placement_heures + categorie_delai (ajoutés 2026-03-13).
    Refonte 2026-03-26 : grain réduit à (agence_id, semaine_debut), categorie_delai retiré,
    ajout pct_moins_4h / pct_meme_jour / pct_plus_3j pour distribution opérationnelle.
    """
    return f"""
    SELECT
        ...
        ROUND(
            COUNT(*) FILTER (WHERE m.delai_placement_heures <= 4)::DECIMAL
            / NULLIF(COUNT(*), 0) * 100
        , 1)                                                       AS pct_moins_4h,
        ROUND(
            COUNT(*) FILTER (WHERE m.delai_placement_heures <= 24)::DECIMAL
            / NULLIF(COUNT(*), 0) * 100
        , 1)                                                       AS pct_meme_jour,
        ROUND(
            COUNT(*) FILTER (WHERE m.delai_placement_heures > 72)::DECIMAL
            / NULLIF(COUNT(*), 0) * 100
        , 1)                                                       AS pct_plus_3j
...
def build_conformite_dpae_query(cfg: Config) -> str:
    """Conformité DPAE par agence/mois — indicateur légal (KPI critique URSSAF).
    ecart_heures = MISS_FLAGDPAE − CNTI_DATEFFET (Silver missions 2026-03-13) :
      <= 0 → DPAE envoyée avant ou à la prise de poste (conforme)
      > 0  → DPAE envoyée après la prise de poste (retard légal)
      NULL → DPAE non transmise (manquante)
    Enrichissement 2026-03-26 : ajout nb_dpae_conformes + nb_dpae_retard,
    taux_conformite_dpae corrigé = conformes / total (non transmises + retard ≠ conformes).
    ecart_moyen_heures restreint aux missions en retard (ecart_heures > 0).
    """
    return f"""
    SELECT
        m.rgpcnt_id                                                            AS agence_id,
        ...
        COUNT(*) FILTER (WHERE m.statut_dpae IS NOT NULL
                           AND m.ecart_heures <= 0)                            AS nb_dpae_conformes,
        COUNT(*) FILTER (WHERE m.statut_dpae IS NOT NULL
                           AND m.ecart_heures > 0)                             AS nb_dpae_retard,
        COUNT(*) FILTER (WHERE m.statut_dpae IS NULL)                          AS nb_dpae_manquantes,
        ROUND(
            COUNT(*) FILTER (WHERE m.statut_dpae IS NOT NULL
                               AND m.ecart_heures <= 0)::DECIMAL
            / NULLIF(COUNT(*), 0),
        4)                                                                     AS taux_conformite_dpae,
        ROUND(AVG(m.ecart_heures) FILTER (WHERE m.ecart_heures > 0),
    ...
    with get_duckdb_connection(cfg) as ddb:
        ...
        _new_tables = {
            "fact_delai_placement": (
                ...
                # Refonte 2026-03-26 : categorie_delai retiré, grain (agence_id, semaine_debut)
                # pct_moins_4h / pct_meme_jour / pct_plus_3j ajoutés
                ["agence_id", "semaine_debut", "nb_missions",
                 "delai_moyen_heures", "delai_median_heures",
                 "pct_moins_4h", "pct_meme_jour", "pct_plus_3j"],
            ),
            "fact_conformite_dpae": (
                build_conformite_dpae_query(cfg),
                # Enrichissement 2026-03-26 : nb_dpae_conformes + nb_dpae_retard ajoutés
                # taux_conformite_dpae corrigé = conformes / total
                ["agence_id", "mois", "nb_missions", "nb_dpae_transmises",
                 "nb_dpae_conformes", "nb_dpae_retard",
