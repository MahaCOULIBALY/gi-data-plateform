"""silver_interimaires_detail.py — Silver · évaluations + coordonnées intérimaires.
...
def process_evaluations(ddb, cfg: Config) -> int:
    ...
    WITH raw AS (
        ...
        FROM read_json_auto('{b}/raw_wtpeval/**/*.json', union_by_name=true, hive_partitioning=false)
    )
    SELECT
...
def process_coordonnees(ddb, cfg: Config) -> int:
    ...
    WITH raw AS (
        ...
        FROM read_json_auto('{b}/raw_pycoordonnee/**/*.json', union_by_name=true, hive_partitioning=false)
    )
    SELECT
